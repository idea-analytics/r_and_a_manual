...
p +
  scale_color_idea()
```
**Super easy!**.  We can also easily change the the palate and even reverse it:
```{r}
p +
  scale_color_idea(palette = "blueorange", reverse = TRUE)
```
Available palette's are given in the `idea_palettes` variable adn can be seen here
...
Note that IDEA's brand colors are not completley safe for color blindness.  Here's each pallete with adjustments to check for the predominant types of color blindness in the population:
