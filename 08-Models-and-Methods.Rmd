...
A **feature** is a essentially a thing in the world. In the simple feature specification a feature ha *a geometry* describing where on Earth the feature is located, **and they have attributes**, which describe other properties, like names ormetrics like population.
...
library(viridis)
tarr_5_9<-tarr %>% 
  filter(variable %in% c("B01001_004", #5-9
                         "B01001_005", #10-14
                         "B01001_006",
                         "B01001_028", 
                         "B01001_028", 
                         ...
                         "B01001_030")) %>% 
  group_by(GEOID, NAME) %>% 
  summarise(tot_5_9 = sum(estimate))


base_stamen_map %>% 
  ggmap() +
  
  geom_sf(data = tarr_5_9, 
         aes(fill = tot_5_9), 
         inherit.aes = FALSE, 
         alpha  = .82, 
         color = NA) +
