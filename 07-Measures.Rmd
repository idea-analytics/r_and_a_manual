...
--join Schools and Regions tables
  FROM [1065574-SQLPRD1].[PROD1].[Schools].[Students] AS A
    LEFT JOIN [1065574-SQLPRD1].[PROD1].[Schools].[Schools] AS B
      ON A.[SchoolNumber] = B.[SchoolNumber]
    INNER JOIN [1065574-SQLPRD1].[PROD1].[Schools].[Regions] AS C
...
TeachBoost is a platform used to collect observations. These are observations of teachers (teachers, co-teachers), and teacher managers (API's, Principals, etc.)

There are 3 key tables in TeachBoost: 
1) Users
2) Forms
3) Templates

Users table is a list of all employees who have access to TeachBoost, whether or not they have ever logged in. The "id" column on Users is a TeachBoost assigned id number. This "id" can be either: 1) an "author id" or 2) a "topic id". An "author id" indicates the person who wrote the form (the observation). The "topic id" indicates who the observation/form is about. If a person from the Users table has an id "123456", and they submitted a Lesson Plan Form, then that "id" from Users matches up with an "author id" on the Forms table. If the same person from Users has id "123456" for GET 1A Form, then that teacher was observed by a manager on a Guidepost rating (GET 1A). The Forms table would have the id "123456" as the "topic id", since the form is about that person. 

In addition, "resource id" on the Users table is the "employee ID" from Tyler Munis (or "NameID" on Employees table), so this column can be used to join to other tables to get more information about the employee. The Users table has a "date last modified" column that shows that last login date and time (or NA if never logged in), it shows the employee's job title (Principal, Math Teacher, SPED Teacher, etc.), their full name, email address.

On the Users table, a person can be listed more than once. For example, some users have 26 rows that have an idential name, work email, and "id" column, but the column called "UserId" is unique, and the numbers increase as the rows increase. This is the number of the times someone has logged in(not that specific user, but from all users of TeachBoost). This poses a problem when joining Users table to Forms table (to see who specifically either authored the Form or the Form was about, since on Forms we only have author id or topic id whic are numbers). Leftjoin adds many, many extra rows, so you first have to take the max(UserId) for each specific user prior to joining the table, in order to be left with one row per person (on the Users table), and not end up with many extra rows after the left join to Forms.

The Forms table is all observation forms ever submitted. These include GET forms (obs of teachers), SLL forms (obs. of teacher leaders), BOY forms, Lesson Plan Submissions forms, and a few more forms. There can be many rows that have the same Form Name, as for example, the GET 1A form (a teacher observation about a Guidepost) can be submitted multiple time for one specific teacher, or many times for different teachers. To get the example name of the Form, you need to join to the Templates table, which is a list of 102 rows (and 101 distinct rows), which means there are 101 different names of Forms. 


The Templates table is a list of all Form Names, with specific details. the Forms table will indicate if it is a GET form but not which specific one. Templates will tell you that is it "GET 1A" or "GET 2C", etc. There are 102 rows of names and 101 distinct names on this table. leftjoin Forms table to Templ
