...
Example: John enrolled in IDEA Public Schools on August 19, 2019, and attended school on the FDOP, which is the Monday after the first week of school. John attended the entire 2019-2020 academic year and was enrolled and attending on the FDOP of the following academic year (FDOP for 2020-2021 school year). If John was enrolled but did not attend on the FDOP because he didn't return until September, John would not count as a student who persisted.
...
During the 2022-2023 School Year the Software Development team created a dashboard to track Staff Retention as a replacement of the weekly reports that Alfredo sent out. This dashboard tracks retention by Region, Campus, School, Area, Chief, Teachers (Only), and by Demographics, which includes reason for leaving, Race/Ethnicity, Tenure, First Year, 90 Day, First Year Teacher, Job Group, and First Year Job Group. The Staff Retention dashboard can be found here: [Staff Retention Dashboard](https://app.powerbi.com/links/1vL4fsQaP_?ctid=02f61eb0-8f78-48cf-beec-40296abdf806&pbi_source=linkShare&bookmarkGuid=4b219ac3-4c9c-4588-9673-4b36a9de016d) (or on the HUB -\> Quick Links -\> Dashboards -\> Locus -\> Staff Retention). If you can't find what you need in the dashboard, the following sections explain the tables that store Staff Retention data.

### StaffHeadCountNew Table
This table contains the current year's staff retention data. 
...
-   **Campus:** Abbreviated Campus long name (very dirty)
-   **Region:** Abbreviated region name (very dirty)
...
-   **SubFunctionalGroup:** Indicates if the employee is National, Campus based, Regional, etc. (Some missingness in this column)
-   **ChiefName:** The abbreviated chief (Cso)
...
+   If staff member 1 were to leave prior to their 91st day of employment, this rate would change to 1-(2/10) = 80%.

#### Tips for Staff Retention Reporting
When calculating year over year retention using reporting.employees, two columns, TRUELEAVER and TRUEHEADCOUNT, need to be created to get the proper retention for the previous year. These columns can be created as follows (from SY 22-23 and SY 23-24; years/dates can be changed as necessary):

```{r yoy retention, echo=TRUE, eval=FALSE}
reporting.employees <-  reporting.employees %>% 
  mutate(TRUELEAVER = case_when(YEAR == "2022" & EXITDATE >= "2022-07-01" & EXITDATE <= Sys.Date()-years(1) ~ 1,
                                YEAR == "2023"& LEAVERS == "1" ~ 1,
                                TRUE ~ 0)) %>%
  mutate(TRUEHEADCOUNT = case_when(YEAR == "2022" & STARTDATE <= Sys.Date()-years(1) ~ 1,
                                   YEAR == "2023" & HEADCOUNT == "1" ~ 1,
                                   TRUE ~ 0))
```

The following R code is an example of calculating All Staff Retention by Entity for the current year and prior year using TRUELEAVER and TRUEHEADCOUNT:
```{r entity ret, echo=TRUE, eval=FALSE}
entity_ret <- reporting.employees %>%
  {list(group_by(.,YEARLONG, ENTITY, REGIONNAME),
        group_by(.,YEARLONG, ENTITY),
        group_by(.,YEARLONG)
  )} %>%
   map_df(., ~{
    .x %>%
    summarise(n_employees = sum(TRUEHEADCOUNT),
              n_leavers = sum(TRUELEAVER),
              n_retained = n_employees-n_leavers) %>%
    mutate(pct_retained = (n_retained/n_employees),
           pct_retained = round(pct_retained, digits = 4)) 
     }) %>%
  mutate(ENTITY = replace_na(ENTITY,"ORG WIDE")) %>%
  distinct() %>%
  mutate(REGIONNAME = ifelse(is.na(REGIONNAME), ENTITY, REGIONNAME)) %>% 
  filter(ENTITY != "NA") %>%
  filter(REGIONNAME == "IPS" | REGIONNAME == "TEXAS" | REGIONNAME == "ORG WIDE") %>% 
  arrange(YEARLONG)
```
