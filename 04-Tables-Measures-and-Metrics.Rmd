...
* **Current Year + 3 Past Years** This table currently has data for the current year (2023-2024), and the last three years data (2022-2023, 2021-2022, & 2020-2021). Once the current year is complete and the next year begins, 2020-2021 data will drop off of this table and the new academic year will be added.
...
* **_[StudentCSI]:_** This table will aid us to identify the students who are/were enrolled in math or reading CSI intervention programs. Since sometimes there is need for data adjustments from one year to another, the [ProgramID] number might change. However, this should not be a problem because we can always use the [InterventionType] field to correctly associate an intervention program to either math or reading.
...
**Step One:** pull down the most recent year's project on GitHub.
**Step Two:** Open the main R project within that folder, for example, "22-23 AP Report & Requests". Within that project there are two main sub-project folders. Use the AP Requests.
**Step Three:** Run the two scripts in the data folder. First run "1_Load_AP_Data", this pulls all of the AP tables. Next run "2_Load_Students_Data", this pulls all of the students and excludes Thrive and Rise students, which are students with severe cognitive disabilities that are over the age of 18 (Thrive) or students with severe cognitive disabilities that do not take AP course work specified by their Individual Education Plan (IEP) but are under the age of 18 (Rise).
...
##Work Start Date:
FORMAT(premppay_all.prep_work_st_dt,'d','en-us')  AS 'Work Start Date'

##Job Status (NLE vs Active)
,CASE WHEN prempmst.prem_term_date < GETDATE() THEN 'NLE'  
...
###New Teacher at IDEA
Start Date and Work Start Date are important columns for determining a flag for New Teacher at IDEA.
To determine if someone is a new teacher (as there is no existing flag or field in the Tyler Munis data), one would have to first look at the Start Date, then look at the Work Start Date. If those years match up, it is most likely a new teacher. But, for example, if the Start Date year does not match the Work Start Date year (Start Date is an older year), then the employee is not a new teacher (if the job roles are the same for both columns, they are both TEACHER). If a teacher's Start Date is July 15, 2020 and their Work Start Date is August 1, 2021, this is not a new teacher (given that the employee's role was "TEACHER" in both those cases). However, it is possible in this example that the person was hired as a co-teacher in 2020, then started in the role of a teacher in 2021. In that case, the employee would not be new to IDEA, but would be a new teacher at IDEA. 
...
![TeachBoost table joins](img/teachboost_table_joins.png "TeachBoost table joins"){width="500"}
