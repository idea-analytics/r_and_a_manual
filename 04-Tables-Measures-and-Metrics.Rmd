...
Within this chapter we will find the tables and metrics IDEA Public Schools stakeholders are the most interested in.

Note that an individual table may not contain all information of interest. This additional information could be extracted from a different table and then joined. Information regarding the various types of joins can be found [here](#section-joins).
...
We use this table to know to what subpopulation a student belongs to. Also, we can link this table to other student data using the *[StudentNumber]* field.
...
-   ***[EnrollmentStatus]:***
    ...
    -   If EnrollmentStatus = 0, the student is/was an active student for the corresponding academic year,
    -   If EnrollmentStatus = 2, the student left the district before completing the academic year,
    -   If EnrollmentStatus = 3, the student graduated from IDEA's high school.

-   ***[ELLCode]:*** English Learner (EL) A.K.A. Limited English Proficiency (LEP)

    -   If ELLCode = 0, the student **is not** identified as an EL student,
    -   If ELLCode = 1, the student **is** identified as an EL student,
    -   If ELLCode not in (0,1), the student is not currently identified as an EL but was identified before as one,
...
-   **EL Reclassification** An EL who has passed the Texas English Language Proficiency Assessment System (TELPAS) will be reclassified as a Non-EL and monitored for 4 years. After reclassification, the student's ELLCode will not change to 0, it will move through the codes for the 4 years of monitoring status and then change to an ELLCode of 5

-   ***[SPED]:*** SPED students are composed of RISE and Life Skills students, not including Critical Student Intervention (CSI) students.

    -   If SPED = 0, the student is not identified as a SPED student, otherwise SPED = 1.

-   ***[InstructionalSettingCode]:*** Students who are classified as SPED may receive additional services. Their services can be found in the **InstructionalSettingDescription** column, which is linked to a specific [PEIMS code](https://tealprod.tea.state.tx.us/TWEDS/98/0/0/0/CodeTable/List/14562):
...
-   ***[EconomicDisadvantageCode]:*** A.K.A ECD
    -   If EconomicDisadvantageCode = 0, then the student is *not identified as economically disadvantaged*, otherwise the student is identified as **economically disadvantaged**,
    -   If EconomicDisadvantageCode = 1, then the student is identified as economically disadvantaged and eligible for free meals,
    -   If EconomicDisadvantageCode = 2, the student is also identified as economically disadvantaged but only eligible for reduced-price meals,
    -   If EconomicDisadvantageCode = 99, it means the student is identified as economically disadvantaged but has another economic disadvantage.
...
-   ***Race / Ethnicity:*** Same like in TX
    -   If the student identifies as Hispanic or Latino, the student is classified as Hispanic/Latino,
    -   If the student **does not** identify as Hispanic or Latino and only selected one race, the student is classified as that race,
    -   If the student **does not** identify as Hispanic or Latino and selected more than one race, the student is classified as two or more races.
-   ***[RowIsCurrent]:*** Indicates the most current row for a student, **Only to be used on the current school year**
...
-   IDEA Amber Creek was renamed IDEA Ambrose and Freda Robinson (short name IDEA Robinson) as of the 2022-2023 school year.
...
IDEA opened new schools in four states over the last 20+ years. IDEA Academy and IDEA College Preparatory in Donna were the first two schools to open in 2000-2001 [see also regions context] (<https://idea-analytics.github.io/r_and_a_manual/key-tables-measures-and-metrics.html#regions-table>).

Need to know what year a school was opened? See the **LaunchSchoolYear** column in the Schools table, Dashboard database. As of fall 2023, this table includes 164 schools with LaunchSchoolYear ranging between 2000-2001 and 2021-2022-2023.
...
-   **[RegionID]:** IDEA region number describing the location of school; links to the Regions table
-   **[CampusTypeID]:** uses values 1 and 2 to indicate campus as Academy or College Prep, respectively
-   **[CountyDistrictCampusNumber]:** number combining the County, District, and Campus numbers, like SchoolNumber
-   **[LaunchSchoolYear]:** school year the school was first opened; ranges from 2000-2001 to 2022-2023

### School Scaling Status

IDEA schools do not typically open with all grade levels present. Each year of operation includes the addition of grade levels until it is fully scaled. Details about where to find the scaling status of the school along with sample R code to pull this information is found below:
...
-   **[RegionDescription]:** region description (e.g., Rio Grande Valley)
...
**Database:** [RGVPDSD-DWPRD1].[PROD1].[Attendance].[Students] \* **Current Year + 3 Past Years** This table currently has data for the current year (2023-2024), and the last three years data (2022-2023, 2021-2022, & 2020-2021). Once the current year is complete and the next year begins, 2020-2021 data will drop off of this table and the new academic year will be added.

**For Attendance Data Prior to 2019-2020:** **Database:** [RGVPDSD-DWPRD1].[PROD1].[Attendance].[StudentsHistorical] \* **Academic Years: 2021-2022 - 2015-2016** This is the official historical table. All of the same columns as the [Attendance].[Students] table. Was told that when the academic year ends the data will roll over to this table.
...
Student Persistence is one of the most important measures that Chiefs, VPs, and School Leaders are always monitoring to understand our efforts in providing high quality services to our students and families. In order for a student to actualize the benefits of IDEA, the student must persist through high school. We say a student persisted if the student was enrolled and active for the entire academic school year, returned the following school year, and was enrolled and active on the First Day of Persistence (FDOP). 
In other words, the student was enrolled and attended school on FDOP for two consecutive years. The only exception to this are New Students that enroll after the FDOP, attend the entire year, return the following year, and are enrolled and active at FDOP. These new students were not enrolled and attending for two consecutive FDOPs because they enrolled after the first FDOP but would still be considered as persisting.

Although it appears the definition of FDOP may have changed over time (previously listed in the manual as the Monday after the first week of school), according to the 22-23 Persistence Dictionary, the FDOP is defined as 11:59 PM on 10th day of school for the current academic year. In practicality, unless you have an automatic data pull at 11:59 PM, the FDOP we use is the 11th day of school. 

::: {.tip}
Unless a holiday falls within the first two weeks of school, the 11th day of class will be 14 calendar days after the First Day of School (FDOS). 

For a quick and easy reference, either pull up your calendar and check the date two weeks down from FDOS, or take FDOS and add 14!
:::


Example: The first day of school at Johns IDEA school was August 12, 2024. John was enrolled and active on August 26, 2024, FDOP for his school (11th day of class). If John remained enrolled and actively attending the entire 2024-2025 academic year and was enrolled and active on the FDOP of the following academic year (FDOP for 2025-2026), he has persisted. If, however, if John was enrolled but was not active (attending) on the FDOP because he didn't actually return until September, John would not count as a student who persisted.

Julia, enrolled and began attending IDEA in October, after moving to a new city. She remained enrolled and actively attending the entire 2024-2025 academic year and was enrolled and active on FDOP of the following academic year (FDOP for 2025-2026). She persisted, even though she was not enrolled on FDOP of 2024-2025.

Finally, if Jose was active at FDOP in 2024-2025, left midyear to attend another school, and then returned to IDEA still within the 2024-2025 academic year, and is still active if at FDOP 2025, he also would be considered to be persisting. 

The only way to get accurate Persistence data is using the official persistence tables. There are many additional rules regarding who is counted in the persistence. For example, graduating seniors do not count towards persistence. Other reasons for exclusion include but are not limited to: the student having a migrant status, death, incarceration, expected but never enrolled, etc. Although relatively rare in numbers, these types of exclusions could impact your calculations if attempting to calculate persistence without the official persistence tables. 

::: {.gotcha}
It is easy to get confused when looking at the current academic year's persistence data or dashboards. The current tables and dashboards are **not** tracking persistence from FDOP last year to FDOP this year. Instead they are tracking persistence from FDOP this year to the current date. You can think of it as Year-To-Date (YTD) persistence. Current persistence will appear much higher than historical persistence, because YTD persistence slowly drops over the course of the academic year, with more distinct drops at certain times of year. 

See the screenshot of a chart from the Persistence Dashboard demonstrating this trend. The 75.17% from 2024-2025 would indicate the final persistence rate for the 2024-2025 academic year. 

![Persistence Chart](img/PersistenceChart.png "Persistence Chart from Persistence Dashboard showing drop over the course of the academic year."){width="500"}
...
**General Formula:** 1 -- (Leavers/All Students) or 1 - (FDOPLEAVERS/FDOPENROLLED + FDOPNEWENTRY)

**Tables:**

-   ***[Fact].[PersistenceCode]:*** This table is used to calculate the persistence rate for the current academic year. The data is updated every three hours, weekdays. LeaverWeek column shows the week the student left the district (beginning the week containing the FDOP, which is the second week of the academic year). After the end of the current school year but before the persistence year has ended (the summer months), the week the student leaves the district is captured in the ULeaverWeek column. The PersistenceWeek column shows the current week of persistence for the school and region that the student attends (this week will be different across schools and regions due to differing start dates and FDOPs).

-   ***[Fact].[PersistenceCodeHistorical]:*** This table contains the final persistence data for previous persistence years starting with 2018-2019 and up through 2022-2023. This table is updated with the previous years' data after that year is closed out and the next persistence year has started.

**Context:**

-   **Who Counts?**

To calculate persistence, we need to filter out all students who should be excluded from the denominator, filter(FDOPCOUNTP == 1). This limits the denominator to only those students that should be counted. This filter should be used when looking at current **or** historical data.
...
-   ***[Colleges]:*** Here we will find all colleges/universities. What makes this table important is the [CEEB] field which is an ID that can help us connect distinct college related data sources like Naviance with National Student Clearinghouse data!

-   ***[CollegeTuition]:*** Besides having in-state tuition and out-of-state tuition data, we can also find two other unique codes ([OPEID] and [ACTCode]), that will aid us to college data across different data sources.

-   ***[EDocs]:*** Details about application submitted e-documents.

-   ***[StudentCollegeApplication]:*** This is the main table. We use it to calculate most of the metrics the College Application & Matriculation dashboard has. We can find data from 2018 up to the current academic year.
...
Critical Student Intervention (CSI) is a program that was introduced in 2014 that aims to provide additional Reading and Math instructional time to students in grades 3-8 ([Hub site](https://ideapublicschoolsorg.sharepoint.com/SpecialPrograms/SitePages/Critical-Student-Intervention.aspx)). Students are selected to participate in the CSI program based on Renaissance STAR (RenStar) or state assessment scores. Students with a gap of 2.0 or more years in either Math and/or Reading as measured by their respective RenStar grade equivalency (GE) are selected into a CSI program. However, based on documentation, students who failed their Reading and/or Math state assessment can also be selected for the CSI program. For students who are brought in after late-June and before the first day of school, a decoding placement test is used (in addition to state assessment score) since RenStar is not available during this time (rollover in our system).

Within the CSI program, there are two intervention subjects (Math and Reading), and each program has various strands. For example, students in CSI Reading are grouped into strands such as Decoding, Reading Mastery, and Corrective Reading. Based on CSI documentation, programs such as DISE and Español to English are identified as neither Math nor Reading; however, in the data, they are labeled under Reading as students who are in DISE tend to move into Decoding A (another Reading intervention) after 60 lessons.

While there is documented guidance on CSI entry, there is limited information on the CSI exit process. For instance, there currently is not a flag in the data warehouse to indicate whether a student has exited CSI programming. An exit date is included in a warehouse table; however, it has not been found to be accurate (e.g., some students with an exit date continued, without a break, in the program). According to CSI documents, students are required to be enrolled in CSI courses for the entirety of the school year, and students exit the program at the end of the school year if their RenStar GE score is less than two years below grade level. However, the data does not reflect these requirements. For instance, students across grade levels 3-7 are exiting CSI despite having end of year RenStar GEs that are more than two grade levels below.

The Software Development Team has created tables in which students who are enrolled in CSI programs can be identified. The tables are described below.
...
This table is used to populate the [CSI Locus dashboard](https://app.powerbi.com/groups/7c5c298d-bf00-4c77-83c3-5d50c8ce7220/reports/ae2401c6-c8bf-4e8e-8988-9142069c131a/ReportSection?ctid=02f61eb0-8f78-48cf-beec-40296abdf806&experience=power-bi&clientSideAuth=0&bookmarkGuid=Bookmark564eb5d1535ec89486c2). It is recommended to cross-reference the data in this dashboard with the raw data in the table as inconsistencies with student counts have been observed.

IDEA data cannot be used to pull data from this table in R, so the following code can be utilized to access the data:
...
It has been noted that some students have multiple grade levels recorded within the same school year. As such, it is recommended to join this table with the Students table (`[RGVPDSD-DWPRD1].[PROD1].[Schools].[Students]`) for the most up-to-date grade level.

#### StudentCSI

`[RGVPDSD-DWPRD1].[PROD1].[Schools].[StudentCSI]`

This table identifies students who are/were enrolled in Math or Reading CSI intervention programs. Since there is need for data adjustments from one year to another, the [ProgramID] number might change. However, this should not be a problem because the [InterventionType] field can correctly associate an intervention program to either Math or Reading.
...
The STAAR assessments are Texas' standardized tests administered every year, except for 2019-2020 due to COVID-19. Grades 3-8 take the Reading and Math assessments every year with Writing, Science, and Social Studies assessed at specific grades. In 2021-2022, the TEA removed the writing assessment and incorporated writing into all assessments. Testing schedule is as follows:
...
Starting in 2022-2023, the STAAR exam was administered online only, causing most IDEA Texas assessments to switch to using Edcite exclusively.
...
For example, a 6th grade ELA semester exam in Fall 2024 may have the name **TX_ELA_6thReading_F24_SE**, or the first AP Spanish Language unit exam may have the name **TX_SPA_APSpanishLanguage_F24_UE1**.

::: gotcha
Note that prior to 2021, many assessments omitted the state abbreviation, so if you need to identify historical exam names, start with the subject code instead.
:::
...
-   **[Mastery]:** 1 = demonstrated mastery (passed, approaches+, etc.); 0 = did not
...
-   **[AssessmentReporingName]:** One of two columns to choose a type of assessment.
-   **[AssessmentType]:** One of two columns to choose a type of assessment.
-   **[QuestionNumber]:** Describes the question and part (1a, 1b, 2, etc.)
...
As IDEA grows nationally, certain regions have assessment-specific platforms that do not directly flow into the data warehouse. For example, Louisiana schools historically used [ANet](https://www.achievementnetwork.org/) to house some of their LEAP-aligned assessments.

#### Common queries

::: tip
Querying assessment data, particularly item data, can be computationally costly. Here are some best practices if you're looking up specific exam data:

1.  Use the **IABWA** table first to filter for the desired **[SchoolTermID], [Subject], [State] and [AssessmentType] (if known)** to identify exam names you need. Then, use the corresponding **[ExternalTestID]** column to filter for exams efficiently.
2.  If you need items from a specific exam, use the **[ExternalTestID]** in the **IABWAItems** table. Filter for the **[SchoolTermID], [State], [ExternalTestID], [QuestionNumber], and [AssessmentType] (if known)** to get item-level data for all students.
:::

Let's say you want all scores from the open-ended questions of the AP English Literature Unit Exam 1 assessment in 2024-2025. Here is one way to approach this problem:

1.  Use the IABWA table to identify the [ExternalTestID] of the assessment.
2.  Use the [ExternalTestID] to filter the IABWA table again.
3.  Use the [ExternalTestID] and [QuestionNumber] to filter the IABWAItems table.

**To find the names of all exams administered within a school year, use the following query:**
...
You learn that Question 13 was particularly difficult, and would like to investigate the item. The IABWAItems table is extremely large, so ONLY query the assessment AND item you are interested in.

**To query the student-assessment-item-level-data, use the following query:**
...
You may want to join this data with the Students, Schools, and Regions tables to get item-level summaries at every organizational level.
...
[Renaissance Star Assessments](https://www.renaissance.com/products/star-assessments/) are a suite of of norm-referenced assessments used by IDEA for both screening (particularly for identifying students in need of [CSI support](#csi)) as well as gauging achievement and growth of students in a way that is agnostic to both grade-level and location (i.e., Texas, Louisiana, Florida, or Ohio).
...
Accessing these data in the warehouse at this juncture is not always obvious. There are number of tables (e.g. `[RGVPDSD-DWPRD2].[PROD2].[Assessments].[LittleStar]`) that are not frequently updated. For Math RenStar, the best table to use is `[RGVPDSD-DWSRC4].[SRC_AR].[dbo].[StarMathV2]` while the best time for Reading RenStar is `[RGVPDSD-DWSRC4].[SRC_AR].[dbo].[StarReadingV2]`.

Students take the RenStar during three testing windows, beginning of year (BOY), middle of year (MOY), and end of year (EOY). [CSI](#csi) students take this exam BOY, MOY, and EOY as well as two additional times, one in the fall (between BOY and MOY) and one in the spring (between MOY and EOY). Note that students can take the exam more than once during each screening window and these tables include all scores.
...
All AP student data is downloaded directly from the College Board's website. Once downloaded it is imported, cleaned, and added to the R&A server. In order to get the data into a usable format, with local student numbers and deduplicated, there are several scripts in the AP project on GitHub that need to be run in a specific order.

**Step One:** pull down the most recent year's project on GitHub. **Step Two:** Open the main R project within that folder, for example, "22-23 AP Report & Requests". Within that project there are two main sub-project folders. Use the AP Requests. **Step Three:** Run the two scripts in the data folder. First run "1_Load_AP_Data", this pulls all of the AP tables. Next run "2_Load_Students_Data", this pulls all of the students and excludes Thrive and Rise students, which are students with severe cognitive disabilities that are over the age of 18 (Thrive) or students with severe cognitive disabilities that do not take AP course work specified by their Individual Education Plan (IEP) but are under the age of 18 (Rise). **Step Four:** Run the three scripts in the munge folder. First run "1_Clean_AP_Data", this script cleans up the AP data, like making sure that if a student takes the same AP Exam two different years and passes both with the same score that they are not counted twice towards the three exams passed for AP Scholars classification. Then run "2_Scaffold_AP_Data", this creates several scaffolds such as the exams taken in the years that they take them. This script results in a base data file that can be used call "ap_df". This file will include duplicate tests if taken in a different year by the same student. There are not many of these students that took the same exam that they had already passed in an attempt to get a better score. Finally run "3_Scaffold_AP_Scholars_Data", this is the same script as #2, but ensures that the file that is produced by the script doesn't include duplicate exams for the calculation of AP Scholars. The resultant file from this last script is called "ap_df_scholars".
...
Useful Fields: • SchoolNbr • SchoolName • LASID • Grade • ELAVoidFlag • ELAAchievement (Advanced, Mastery, and Basic scores represent passing that subject test) • MathVoidFlag • MathAchievement (Advanced, Mastery, and Basic scores represent passing that subject test) • SocialVoidFlag • SocialAchievement (Advanced, Mastery, and Basic scores represent passing that subject test) • ScienceVoidFlag • ScienceAchievement (Advanced, Mastery, and Basic scores represent passing that subject test)

## Student Course Grades

There are two tables in the warehouse that collect student course grades, `[RGVPDSD-DWPRD1].[PROD1].[Schools].[StudentCurrentGrades]` and `[RGVPDSD-DWPRD1].[PROD1].[Schools].[StudentHistoricalGrades]`. The [StudentCurrentGrades] table is where course grades land from PowerSchool for students in 8th through 12th grade. This table is used to calculate *On track to Graduate* and to check if students are passing their courses. These grades change! After the quarter is complete the grades are moved to the [StudentHistoricalGrades] table as a permanent grade. The [StudentHistoricalGrades] table contains grades for all students in Pre-K though 12th. This is the table used for transcripts and report cards.
...
During the 2022-2023 School Year the Software Development team created a dashboard to track Staff Retention as a replacement for the previous the weekly reports. This dashboard tracks retention by Region, Campus, School, Area, Chief, Teachers (Only), and by Demographics, which includes reason for leaving, Race/Ethnicity, Tenure, First Year, 90 Day, First Year Teacher, Job Group, and First Year Job Group. The Staff Retention dashboard can be found here: [Staff Retention Dashboard](https://app.powerbi.com/links/1vL4fsQaP_?ctid=02f61eb0-8f78-48cf-beec-40296abdf806&pbi_source=linkShare&bookmarkGuid=4b219ac3-4c9c-4588-9673-4b36a9de016d) (or on the HUB -\> Quick Links -\> Dashboards -\> Locus -\> Staff Retention). 

If you can't find what you need in the dashboard, the following sections explain the tables that store Staff Retention data.

### Staff HeadCount New Table
The StaffHeadCountNew table contains the current year's staff retention data. 
...
-   **JobGroup:** Short list of categories that employee's job falls into, such as Clerical/Technical, Teacher, Manual Trades, etc.
-   **AcademicYear:** The current academic year
-   **PositionEndDate:** The position end date, for teachers it is the last day of the previous academic year, such as 2023-06-30
-   **TerminatedDate:** The date the employee was terminated else is NULL
-   **Ethnicity:** The Race/Ethnicity in mutually exclusive categories, however, More than 2 Races is not an option
...
-   **RowIsCurrent:** Indicates that is the most current row of data for that employee
-   **FunctionalGroup:** The division the employee is in
-   **SubFunctionalGroup:** Indicates if the employee is National, Campus based, Regional, etc.
-   **ChiefName:** The abbreviated chief 
-   **DeptRename:** Department name (Instructional, Campus Operations, etc.)
...
For prior year's Staff Retention data there are other tables with the same columns as described above. 

**2018-2019 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew1819] 

**2019-2020 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew1920] 

**2020-2021 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew2021] 

**2021-2022 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew2122]

**2022-2023 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew2223]

**2023-2024 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew2324]

**2024-2025 Table:** [RGVPDSD-DWPRD1].[Staffing].[dbo].[StaffHeadCountNew2425]

Data has varied across the years and the naming conventions (or lack there of) is varied across these tables.

The data that feeds the Staff Retention Dashboard is found in a different table which contains the current year and the prior year's data.

### Reporting Employees Table
The Reporting.Employees table contains both the current and prior year's staff retention data. This is the data that feeds the Staff Retention Dashboard. 
...
-   **JOBTITLE:** Employee's Job Title
-   **SUBFUNCTIONAL GROUP:** Indicates whether the employee's position is Campus-based, Regional, or National
-   **JOBGROUP:** Short list of categories that employee's job falls into, such as Clerical/Technical, Teacher, Manual Trades, etc.
...
-   **AREA:** Area name (Academic Team, Information Systems & Technology, etc.)
-   **DIVISION:** Division name (Curriculum-Instruction-Assessment, Technology Operations, etc.)
-   **DEPARTMENT:** Department name (Instructional, Campus Operations, etc.)
...
-   **TEACHERSTEP:** The step level of the teacher
-   **HEADCOUNT:** 0/1 indicating if the employee is counted in the overall staff headcount
-   **LEAVER:** 0/1 indicating if the employee is considered a leaver
-   **HEADCOUNTFIRSTYEAR:** 0/1 indicating if the employee is in their first year 
-   **LEAVERS90DAY:** 0/1 indicating if the employee left within 90 calendar days of their start date

There are more columns in this table that could be useful, for example there are columns that indicate if the employee is a First Year Teacher or a leaver within the first year of employment.

#### 90 Day Retention
90 Day retention is 1-(number of first year staff who left prior to reaching 90 days after their start date / total number of first year staff).

In the reporting.employees table there is a binary column "LEAVERS90DAY" that is 1 if that staff member left within 90 calendar days of their start date and 0 otherwise. To calculate 90 Day retention, we divide the number of 0's in that column by the total count of first year staff.

**Example:** Total staff headcount: 10

+   1 staff member is actively employed but is only 45 calendar days from their start date (staff member 1) 
+   1 staff member is no longer employed and quit 60 calendar days after their start date (staff member 2) 
+   1 staff member is no longer employed and quit more than 90 calendar days after their start date 
+   7 staff members are actively employed and more than 90 calendar days from their start date

In this example, 90 Day retention would be 1-(1/10) = 90%. 
If staff member 1 remains employed through their first 90 calendar days of employment, this rate would remain unchanged. 
However, if staff member 1 were to leave prior to their 91st day of employment, this rate would change to 1-(2/10) = 80%.

#### Tips for Staff Retention Reporting

When comparing current year-to-date retention to last year's retention using reporting.employees data, two helper columns (TRUELEAVER and TRUEHEADCOUNT) are needed to ensure accurate year-over-year comparisons. 

- **TRUELEAVER** identifies employees who exited during the current retention year up to the cutoff date as well as employees who exited during the equivalent period in the prior retention year. 
- **TRUEHEADCOUNT** identifies employees who are part of the staff headcount during the retention year up to the cutoff date, for both the current year and the equivalent period in the prior year. 

These columns can be created as follows (example uses SY 22-23 and SY 23-24 with cutoff date of May 1st for both years; years/dates can be changed as needed):
...
Both of the previously mentioned staff retention tables (StaffHeadCountNew and Reporting.Employees) contain data for full-time staff only. The All Staff Retention table contains retention data for both full-time and part-time employees. 
...
-   **JobGroup:** Short list of categories that employee's job falls into, such as Clerical/Technical, Teacher, Manual Trades, etc.
-   **AcademicYear:** The current academic year
-   **Race:** The Race/Ethnicity in mutually exclusive categories, however, More than 2 Races is not an option
-   **TerminationDate:** The date the employee was terminated else is NULL
-   **TerminatedReason:** Termination Reason
-   **ManagerName:** Employee's Manager
-   **RowIsCurrent:** Indicates that is the most current row of data for that employee
-   **Functional:** The division the employee is in
-   **SubFunctional:** Indicates if the employee is National, Campus based, Regional, etc. 
-   **Chief:** The abbreviated chief
-   **ManagerID:** Manager's Employee ID
-   **Gender:** M == Male, F == Female
...
Regrettable separation is defined as a separation that IDEA did not want to occur. Non-regrettable separation is defined as a separation that is tolerable for IDEA.

For a separation to be classified as regrettable, the leaver must have one of the following exit reasons:

-   **Voluntary Resignation No Notice**
-   **Voluntary Resignation With Notice**
-   **Voluntary No Return From Leave**

For a separation to be classified as non-regrettable, the leaver must have one of the following exit reasons:
...
### High Stakes Roles
There are some high stakes roles in the vw_CurrentStaffDirectory table located in the Surveys database on the RGVPDRA-DASQL server. However, that information is incomplete. To get high stakes roles from a job title column:
...
When defining high stakes roles with job titles, it is important to go in this order so that groups that overlap are properly assigned (i.e., VP of Schools and HQ VPs). Make sure to apply the proper sentence case for the dataset being used. Keep in mind specialty titles that may contain words that you filter by but do not belong in that grouping, such as Flex Teacher or Teacher Residency Coach.
...
Tyler Munis can be accessed by Maura Carter. It has the munprod database that contains a variety of tables and views for staffing data. One of the main tables for employee data is the prempmst table. Another is the premppay_all view (under munprod), and it has the employee's step, salary, pay rate, pay schedule, latest step "date" (for current step), and employee ID number. This data can be filtered for job type. For example, if you want to see Teacher Salary, you can filter for job type Teacher.

Currently, this data is being used for the Teacher Tenure project for Martin Winchester and the HA team, to investigate if there is a relationship between student achievement (via Semester Exams and STAAR testing scores) and a teacher's Step (pay scale). The higher the step indicates more experience teaching, which includes teaching at IDEA and prior years of experience teaching elsewhere.
...
The prempmst table is one of the primary sources of employee data and information. It has the employee ID number, the step level (for salary), the social security number (which is why the data is deemed more sensitive), start date at IDEA, work start date for that school year, gender, race, ethnicity, and other pertinent employee information and data (much of which has been captured in the HRIS data reports that are sent out weekly). It also includes employee full name, marital status, home phone number, spouse, bank account, supervisor, IDEA email address, alternate email address (the personal email the employee used likely to apply to IDEA), pay type, pay frequency, sick bank.

Prempmst is one of hundreds of tables. It is accessed via this path on MSSQL: RGVPDEIS-TMTXDB\TYLERCI -\> Databases -\> munprod -\> Tables -\> dbo.prempmst_orig

Here are some specific columns or how to format them:

##Work Start Date: FORMAT(premppay_all.prep_work_st_dt,'d','en-us') AS 'Work Start Date'

##Job Status (NLE vs Active) ,CASE WHEN prempmst.prem_term_date \< GETDATE() THEN 'NLE'\
ELSE 'Active' END AS [JobPaySkill]

It is based on term dates. NLE is for employees who are no longer with IDEA, and Active means the employee is an active employee who is still currently working with IDEA. There is a Job Status column on the Retention Reports that could be filtered for NLE or Active in order to calculate retention rates and see who used to be an employee and is no longer, the date the employee left, and who is still a current active employee. (Active employees in the Job Status column are then used on the HRIS reports).

##Start Date

,FORMAT(prempmst.prem_hire,'d','en-us') [StartDate]

Start Date is a column used in the HRIS reports that denotes the date of hire at IDEA (when did the employee start their employment at IDEA, regardless of their current IDEA role now.) For example, an employee could have started for the first time at IDEA on August 1, 2016 in a role as a teacher, but now their current Work Start date is August 1, 2023 as a Principal. This employee would have started at IDEA in 2016 but is a current employee in 2023 under a different role. However, that person doesn't have to switch roles, and the Work Start Date is the start date for their current school year's job role for that particular school year. An employee would have a new Work Start Date each school year (for either the same role or their new current role, if it is different), but would have only one Start Date at IDEA with the organization.

###New Teacher at IDEA Start Date and Work Start Date are important columns for determining a flag for New Teacher at IDEA. To determine if someone is a new teacher (as there is no existing flag or field in the Tyler Munis data), one would have to first look at the Start Date, then look at the Work Start Date. If those years match up, it is most likely a new teacher. But, for example, if the Start Date year does not match the Work Start Date year (Start Date is an older year), then the employee is not a new teacher (if the job roles are the same for both columns, they are both TEACHER). If a teacher's Start Date is July 15, 2020 and their Work Start Date is August 1, 2021, this is not a new teacher (given that the employee's role was "TEACHER" in both those cases). However, it is possible in this example that the person was hired as a co-teacher in 2020, then started in the role of a teacher in 2021. In that case, the employee would not be new to IDEA, but would be a new teacher at IDEA.

### TeachBoost Data

TeachBoost has observation data for teachers and leaders, as well as lesson plan submission and other useful information. The main reason our department has been accessing TB data is for GET observations for teachers and SLL observations for Leaders.
...
forms_try \<- get_table(.table_name = "Forms", .database_name = "SRC_Teachboost", .schema = "dbo", .server_name = "RGVPDSD-DWSRC3") %\>% select(template_name, template_id, id, topic_ids, author_ids, group_ids, status, type_label, type, date, last_modified) %\>% filter(type_label == "GET Row Rating") %\>% filter(date == "2022-08-01" \| date == "2022-08-02" \| date == "2022-08-03" \| date == "2022-08-04" \| date == "2022-08-05" \| date == "2022-08-06" \| date == "2022-08-07" \| date == "2022-08-08" \| date == "2022-08-09" \| date == "2022-08-10" \| date == "2022-08-11" \| date == "2022-08-12" \| date == "2022-08-13" \| date == "2022-08-14" \| date == "2022-08-15" \| date == "2022-08-16" \| date == "2022-08-17" \| date == "2022-08-18" \| date == "2022-08-19" \| date == "2022-08-20" \| date == "2022-08-21" \| date == "2022-08-22" \| date == "2022-08-23" \| date == "2022-08-24" \| date == "2022-08-25" \| date == "2022-08-26" \| date == "2022-08-27" \| date == "2022-08-28" \| date == "2022-08-29" \| date == "2022-08-30" \| date == "2022-08-31" \| date == "2022-09-01" \| date == "2022-09-02" ) %\>% collect() forms_try

Forms table has the topic_id, which is the TB ID of the person about whom the form is written. The author_id is the TB ID of the person who wrote the form. The form type describes whether it is GET, Lesson Plan, SLL, etc. The topic_ID and author_ID can be the same, if the same person wrote the form about themselves, and this number matches the "ID" number on the Users table. The ID from users is not the employee ID; rather, it is the Teachboost user ID number (uniquely) assigned to each person in Teachboost. If you find a form (which is generally an observation that is submitted, such as GET) in the Forms table, to see who it is about (to find the employee ID number), you have to connect the Forms table to the User table by topic_id, then find the employee ID column on the users table. From there, you can connect this joined table to the employees table by the employee ID (found on the Users table) and on the staffing/employee table, to get additional information such as Region or School, for example.
...
To link the TeachBoost Users, Scores, and Forms tables, a key is necessary for each join. Below is a visualization of how to link these tables as well as sample code. For additional information about these tables, please see the [TeachBoost support guide](https://support.teachboost.com/hc/en-us/articles/8083873489940).

![TeachBoost table joins](img/teachboost_table_joins.png "TeachBoost table joins"){width="600"}
...
-   2x2s in TwoByTwos
    ...
    -   Question_ID
    -   Year
    -   SortOrder (specific to year and Reviewer and is order displayed in Cornerstone)
    -   Reviewer
...
-   Goals
-   External Position (not used, but for user added data not captured in Jobvite)
...
-   If someone got a 4.5+ composite TCP score for two consecutive years in a row, they can move to a Level 5 TCP without having the required years of Experience.
...
-   If a person is missing one of the other four TCP components, such as Parent Survey, the points that were from that score are reallocated to the other components, weights are the in the same ratios as the other components (so it's not an equal allocation, meaning you don't divide the percentage four equal ways).

1.  SY 18-19\

-   Available from www.tcp.ideapublicschool.org. Click on "Export", and click the SY you are interested in. Alex Saldivar can give you access; however Maura, Ilissa, Steven, Edison, and Marlena have been granted access. Maura has requested access for Chris, Karina, Mishan, and Aline.
...
If you would like to download your own TCP Export data for SY 19-20, please go to the TCP Application Website: www.tcp.ideapublicschools.org.

3.  SY 21-22 -- please download export from www.tcp.ideapublicschools.org.
...
For all School Years, an "appeals" process takes place in the fall after the original placements are submitted in October. There has been a roughly 50% appeal rate (50% of teachers or their managers appealing the placements) each year. Appeals go before the appeals committee, where some appeals are denied (the given placement holds), and some appeals are approved (so the placement changes). This can alter the TCP Export report, which is not static. The export report changes depending on the current day and month; if you would like to know the "original" placements prior to appeals, you need to download the TCP Export data directly from the tcp application website prior to the appeals and any changes that will be made to placement levels. It is a good idea to download the TCP Export each year in the beginning of October when it is first posted so that you have that data, in case it changes as time goes on.

#### Data Matching

-   Jobvite 19-20 to Teacher Export 19-20
-   Jobvite 20-21 to Teacher Export 20-21 Jobvite 21-22 to Teacher Export 21-22 -- Can't do this! TE 21-22 NOT available yet!
-   Thus, we will be looking at 2 years of data:
    -   Jobvite 19-20 to Teacher Export 19-20
    -   Jobvite 20-21 to Teacher Export 20-21 + Removing Jobvite 19-20 Hires who did not have a Start Date of Fall 2019 + Removing Jobvite 20-21 Hires who did not have a Start Date of Fall 2020 + Removing all Teacher Export 19-20 and Teacher Export 20-21, who did not have a Job Title of Teacher + Removing all Teacher Export 19-20 and Teacher Export 20-21, who had Internal Role as:
-   SPED CP
-   SPED AC
-   PE CP
...
How to join? \* Jobvite has no Employee ID & no Work Email and Teacher Export has Employee ID and Work Email. \* Possibly join through PROD1.Staffing.Employees table, which contains First Name, Last Name, Birthday, Employee ID, and Work Email
...
We are looking at Area = "TEACHER" for 20-21 SY. Then inner_join to all employees on the HRIS for 21-22 SY. There were a total of 2,388 employees who worked for both school years (20-21 and 21-22 SY), including as a Teacher for 20-21 SY and as either a Teacher or different Area for 21-22 SY. We found 39 Job Titles for Areas other than "TEACHER" on the 21-22 SY report (for teachers who had changed Areas from 20-21 to 21-22 SY). One Job Title was "INTERVENTIONIST" which moved from Area = "TEACHER" to Area = "INSTRUCTIONAL SUPPORT". We concluded this was incorrect and should remain as Area = "TEACHER", and so was counted as "no change". The remaining 38 Job Titles that fell outside of Area = "TEACHER" were included as either Promotions or Demotions.

**Promotions**

LICENSED SPECIALIST IN SCHOOL PSYCHOLOGY INTERN ASST. PRINCIPAL OF INSTRUCTION REGIONAL ATHLETICS MANAGER COLLEGE COUNSELOR SPECIAL PROGRAMS COACH DI PROGRAM MANAGER REGIONAL SPECIAL PROGRAMS COACH DIRECTOR OF COLLEGE COUNSELING

ASSISTANT PRINCIPAL OF INSTRUCTION SCHOOL COUNSELOR STUDENT SUCCESS ADVISOR EDU DIAGNOSTICIAN REGIONAL INTERVENTION COACH ASST. PRINCIPAL OF OPERATIONS TALENT RECRUITER PRINCIPAL OF INSTRUCTION PUBLIC RELATIONS MANAGER

EDUCATIONAL DIAGNOSTICIAN TALENT PARTNER MATH CURRICULUM WRITER SPANISH LANGUAGE ARTS CURRICULUM MANAGER ASSISTANT PRINCIPAL OF OPERATIONS IN RESIDENCE ASSISTANT PRINCIPAL OF OPERATIONS ELA SECONDARY CURRICULUM MANAGER STATE REPORTING COORDINATOR

ASST. PRINCIPAL OF INSTRUCTION IN RESIDENCE SOCIAL STUDIES CURRICULUM MANAGER K-2 REGIONAL MATH COACH MATH CURRICULUM MANAGER DIGITAL COMMUNICATIONS MANAGER SR. COLLEGE COUNSELOR TECHNICAL TRAINER PROJECT MANAGER JOB COACH

**Demotions** FLEX TEACHER TESTING COORDINATOR SUBSTITUTE TEACHER SPECIAL EDUCATION RISE CO-TEACHER
...
TCP Placement Levels do not have a high correlation with TCP Composite Scores. The Composite Score is composed of five components (GET Ratings, Student Achievement, Parent Surveys, Student Surveys, and Core Values). This score is loosely used to decide a teacher's Placement Level, but can be overridden by a manager, a "hold", or an appeal (generally initiated by the manager). For teachers who do not get receive a high enough Composite Score to reach the TCP Placement Level they reached the prior year, a "hold" is placed to keep the teacher at the prior year's level (given that it is higher than this year's Composite Score). The teacher then has one academic year to get their Composite Score raised high enough to keep their TCP Placement Level. During the Pandemic, teachers were given two years (and then now, indefinite years of pandemic holds) of "hold" time instead of one. General cut-offs are: 0 to 1.4 rounds to Level 1, 1.5 to 2.4 rounds to Level 2, 2.5 to 3.4 rounds to Level 3, 3.5 to 4.4 rounds to Level 4, and 4.5+ rounds to Level 5. There is also a hold for changing content. If a teacher was teaching chemistry and is now teaching physics, that teacher would be placed on a hold for one year. Some teachers were on hold for 2-3 years prior to the pandemic 19-20 SY, according to the TCP Team, and thus some teachers have been on a TCP hold for 5-6 years at this point (end of 21-22 SY). Business rules may change or be updated and added to again, for the 22-23 SY.
...
Accessing and working with data from the Travis campus in Permian Basin comes with some unique challenges. If you aren’t aware of the unique context, it can be easy to end up with apparently “missing” data from Travis that may actually exist.

Historically and at the time of this writing (Fall 2025), IDEA Travis has a unique partnership with Midland ISD (MISD). Although IDEA operates Travis, it is considered to be part of MISD, and therefore much of Travis data is provided to us by MISD rather than being collected and stored in the same manner as other IDEA Texas data.

Although Travis, like other IDEA Campuses, has an Academy (Elementary) and College Prep (Middle and High School), each with unique school IDs, much of the Travis data is linked only to Travis Academy. For example, TEA treats the Travis campus as a single school (Academy), and therefore data such as accountability ratings and STAAR assessment scores appear only for Travis Academy, despite some being for older grades. When pulling STAAR scores, if you were to filter to grades K-5 for all Academy schools, you would miss STAAR scores for grades 6-12 from Travis College Prep, because they are stored as Academy.

Another common challenge comes with joining Travis data using school IDs. The school IDs for Travis within the schools table are 3-digit IDs, while the remainder of the schools use a 9-digit state school number. In many (most?) cases, the data stored in other tables use Travis’ 9-digit state school number.

Please check school IDs in data you intend to join and then check that the presence of Travis data matches your expectations after any joins.

You may use a variation of the following R code to create the state school number prior to using it for joining.
...
It is possible that the information needed to conduct an analysis resides in more than one table in the data warehouse. To solve for this, a join statement can be used. The appropriate join depends on your data needs and the table structure. In the examples below, five joins will be covered: one inner and four outer joins (left, right, full, and anti). For each join, a matching key (common field between the tables) is identified. Some common keys with IDEA data are StudentNumber and SchoolNumber.

### Inner Join

An `inner join()` retains observations when a matching key value is found in both `x` and `y`. Any unmatched observations are not included in the output. Visually,

![inner_join(x, y)](img/inner_join.png "visualization of an inner join"){width="500"}

### Left Join

A `left_join()` is an outer join that retains all observations from `x`. Any observation in `x` that does not have a matching key in `y` results in NAs in the columns added from `y`. Any observation in `y` that does not have a matching key in `x` is not included in the output. Visually,

![left_join(x, y)](img/left_join.png "visualization of a left join"){width="500"}

### Right Join

A `right_join()` is an outer join that retains all observations from `y`. Any observation in `y` that does not have a matching key in `x` results in NAs in the columns from `x`. Any observation in `x` that does not have a matching key in `y` is not included in the output. Visually,

![right_join(x, y)](img/right_join.png "visualization of a right join"){width="500"}

### Full Join

A `full_join()` is an outer join that retains all observations from `x` and `y`. Any observation in `x` that does not have a matching key in `y` results in NAs in the columns added from `y`. Any observation in `y` that does not have a matching key in `x` results in NAs in the columns from `x`. Visually,

![full_join(x, y)](img/full_join.png "visualization of a full join"){width="500"}

### Anti-Join

An `anti_join()` is an outer join that retains all observations from `x` that do not have a matching key in `y`. Any observation in `x` that does not have a matching key in `y` is not included in the output. Visually,

![anti_join(x, y)](img/anti_join.png "visualization of an anti-join"){width="500"}
