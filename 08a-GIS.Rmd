...
The results can now be transferred to a matrix. To begin, 1s are included in the diagonal of the matrix and then the remainder of the matrix is completed.
![AHP Matrix](img/ahp_matrix.png "AHP Matrix"){width="400"}

To accomplish this in R, the following code can be used:
```{r AHP_matrix, eval = FALSE}
# create 3x3 matrix with all 1s
AHP_matrix <- matrix(1,
                     nrow = 3, 
                     ncol = 3)
#       [,1] [,2] [,3]
...
# add the rankings in the upper triangle
...
# put reciprocal in the lower triangle of the matrix
...
The CI is computed
![CI Formula](img/ahp_ci_formula.png "CI Formula"){width="100"}

where λ^max = largest eigenvalue of the matrix and *n* = the number of criteria.

```{r CI, eval = FALSE}
CI <- (Mod(eigen(AHP_matrix)$values[1]) - 3)/ 2
# [1] 0.04700755

# RCI for n = 3 criteria is 0.58
CR <- CI / 0.58 
# [1] 0.08104751
```
