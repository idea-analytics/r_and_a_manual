...
```{bash branch_one_line-5a, eval=FALSE}
...
```{bash lfs install-5a, eval = FALSE}
git lfs install
```

4.  Track the file extension (in our scenario, .pbix), using

```{bash lfs track-5a, eval = FALSE}
git lfs track "*.pbix"
```

5.  Track .gitattributes

```{bash add gitattributes-5a, eval = FALSE}
...
```{r set_working_directory-5a, eval=FALSE}
...
```{r set_up_project_template-5a, eval=FALSE}
...
```{r install_ideadata_ideacolors-5a, eval=FALSE}
...
```{r install_github-5a, eval=FALSE}
